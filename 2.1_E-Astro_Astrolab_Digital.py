...
def update_star_map():
    ...
    # Tambahkan rasi bintang dengan marker segitiga, posisi dinamis mengikuti lokasi pengguna
    # Daftar rasi dengan RA dan Dec (dalam derajat)
    constellations = {
    ...
    # Untuk posisi dinamis, buat FixedBody untuk masing-masing dan compute dengan observer
    star_info = "Koordinat Rasi Bintang (dalam RA, Dec °):\n"
    for name, data in constellations.items():
        ...
        x_star = math.cos(alt) * math.cos(az)
        y_star = math.cos(alt) * math.sin(az)
        z_star = math.sin(alt)
